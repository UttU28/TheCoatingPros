...
const projectImages = [
  {
    id: 1,
    src: "https://images.unsplash.com/photo-1621878135994-8b56a93927bb",
  ...
  {
    id: 4,
    src: "https://images.unsplash.com/photo-1605521044496-018719d1aee8",
  ...
  {
    id: 7,
    src: "https://images.unsplash.com/photo-1613244439060-76ef6bbb6c3e",
...
export default function ProjectGallery() {
  ...
  const nextSlide = () => {
    setCurrentSlide((prevSlide) => 
      prevSlide === Math.floor(projectImages.length / 3) - 1 ? 0 : prevSlide + 1
    );
  };
  
  const prevSlide = () => {
    setCurrentSlide((prevSlide) => 
      prevSlide === 0 ? Math.floor(projectImages.length / 3) - 1 : prevSlide - 1
    );
  };
      ...
      <div className="container-custom">
        ...
        >
          ...
          <p className="text-lg text-slate-600 dark:text-slate-300 max-w-3xl mx-auto">
            Browse our portfolio of successful commercial roof coating projects. Swipe through to see our work and click on any image for a closer look.
          </p>
        </motion.div>
        ...
        >
          <div className="overflow-hidden">
            ...
            >
              {Array.from({ length: Math.ceil(projectImages.length / 3) }).map((_, slideIndex) => (
                <div key={slideIndex} className="min-w-full">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {projectImages.slice(slideIndex * 3, slideIndex * 3 + 3).map((image) => (
                      <div 
                        key={image.id} 
                      ...
                      >
                        ...
                        <div className="p-4 bg-slate-50 dark:bg-slate-700">
                          <h3 className="text-lg font-semibold text-slate-800 dark:text-white">{image.caption}</h3>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="flex items-center justify-center space-x-2 mt-6">
            {Array.from({ length: Math.ceil(projectImages.length / 3) }).map((_, index) => (
              <button
                key={index}
